...
function Products({ data, products }: { data: DealWorkspaceData; products: Product[] }) {
  const router = useRouter(); const [pending, startTransition] = useTransition(); const [error, setError] = useState<string | null>(null); const [editingId, setEditingId] = useState<string | null>(null); const emptyLine = { productId: "", description: "", size: "", quantity: "", unit: "MT", net: "", gross: "", purchase: "", sale: "", purchaseCurrency: data.deal.purchase_currency ?? "USD", salesCurrency: data.deal.sales_currency ?? "USD", notes: "" }; const [form, setForm] = useState(emptyLine); const set = (key: keyof typeof form, value: string) => setForm((current) => ({ ...current, [key]: value }));
  return <div className="space-y-5"><div className="overflow-x-auto rounded-lg border border-card-border bg-card"><table className="w-full min-w-[850px] text-left text-sm"><thead><tr className="border-b border-border text-xs text-muted-foreground"><th className="p-3">Product</th><th className="p-3">Size/grade</th><th className="p-3">Quantity</th><th className="p-3">Net/Gross</th><th className="p-3">Purchase</th><th className="p-3">Sales</th><th className="p-3">Line amounts</th><th className="p-3">Actions</th></tr></thead><tbody className="divide-y divide-border">{data.products.map((item) => <tr key={item.id}><td className="p-3"><p className="font-medium">{item.product_name ?? item.product_description ?? "Custom product"}</p><p className="text-xs text-muted-foreground">{item.sku ?? item.scientific_name ?? "—"}</p></td><td className="p-3">{item.size_grade ?? "—"}</td><td className="p-3">{item.quantity} {item.unit}</td><td className="p-3">{item.net_weight ?? "—"} / {item.gross_weight ?? "—"}</td><td className="p-3">{item.purchase_price ?? "—"} {item.purchase_currency ?? ""}</td><td className="p-3">{item.sales_price ?? "—"} {item.sales_currency ?? ""}</td><td className="p-3"><p>Purchase: {item.purchase_price == null ? "—" : lineAmount(item.quantity, item.purchase_price)} {item.purchase_currency}</p><p>Sales: {item.sales_price == null ? "—" : lineAmount(item.quantity, item.sales_price)} {item.sales_currency}</p></td><td className="p-3"><button type="button" disabled={pending || Boolean(data.deal.archived_at)} className="mr-3 text-xs underline" onClick={() => { setEditingId(item.id); setError(null); setForm({ productId: item.product_id ?? "", description: item.product_description ?? "", size: item.size_grade ?? "", quantity: String(item.quantity), unit: item.unit, net: item.net_weight == null ? "" : String(item.net_weight), gross: item.gross_weight == null ? "" : String(item.gross_weight), purchase: item.purchase_price == null ? "" : String(item.purchase_price), sale: item.sales_price == null ? "" : String(item.sales_price), purchaseCurrency: item.purchase_currency ?? "", salesCurrency: item.sales_currency ?? "", notes: item.notes ?? "" }); }}>Edit</button><button type="button" disabled={pending || Boolean(data.deal.archived_at)} className="text-xs underline" onClick={() => { if (!window.confirm("Remove this product line?")) return; startTransition(async () => { const result = await removeDealProduct(data.deal.id, item.id); if (!result.success) return setError(result.error); setEditingId(null); setForm(emptyLine); router.refresh(); }); }}>Remove</button></td></tr>)}</tbody></table>{!data.products.length ? <p className="p-6 text-center text-sm text-muted-foreground">No product lines.</p> : null}</div><form className="space-y-4 rounded-lg border border-card-border bg-card p-4" onSubmit={(event) => { event.preventDefault(); startTransition(async () => { const input = { notes: form.notes, business_case_id: data.deal.id, product_id: form.productId || null, product_description: form.description || null, size_grade: form.size || null, quantity: toNumber(form.quantity) ?? 0, unit: form.unit, net_weight: toNumber(form.net), gross_weight: toNumber(form.gross), purchase_price: toNumber(form.purchase), sales_price: toNumber(form.sale), purchase_currency: form.purchaseCurrency || null, sales_currency: form.salesCurrency || null }; const result = await (editingId ? updateDealProduct(editingId, input) : addDealProduct(input)); if (!result.success) return setError(result.error); setError(null); setEditingId(null); setForm(emptyLine); router.refresh(); }); }}><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Field label="Catalog product"><select className={inputClass} value={form.productId} onChange={(e) => set("productId", e.target.value)}><option value="">Custom description</option>{products.map((item) => <option key={item.id} value={item.id}>{item.sku} — {item.name}</option>)}</select></Field><Field label="Description"><input className={inputClass} value={form.description} onChange={(e) => set("description", e.target.value)} /></Field><Field label="Size / grade"><input className={inputClass} value={form.size} onChange={(e) => set("size", e.target.value)} /></Field><Field label="Quantity / unit"><div className="grid grid-cols-[1fr_90px] gap-2"><input required type="number" min="0" step="any" className={inputClass} value={form.quantity} onChange={(e) => set("quantity", e.target.value)} /><select className={inputClass} value={form.unit} onChange={(e) => set("unit", e.target.value)}>{["MT", "kg", "unit", "carton"].map((unit) => <option key={unit}>{unit}</option>)}</select></div></Field><Field label="Net / gross weight"><div className="grid grid-cols-2 gap-2"><input type="number" min="0" step="any" className={inputClass} value={form.net} onChange={(e) => set("net", e.target.value)} /><input type="number" min="0" step="any" className={inputClass} value={form.gross} onChange={(e) => set("gross", e.target.value)} /></div></Field><Field label="Purchase price / currency"><div className="grid grid-cols-[1fr_80px] gap-2"><input type="number" min="0" step="any" className={inputClass} value={form.purchase} onChange={(e) => set("purchase", e.target.value)} /><input className={inputClass} maxLength={3} value={form.purchaseCurrency} onChange={(e) => set("purchaseCurrency", e.target.value.toUpperCase())} /></div></Field><Field label="Sales price / currency"><div className="grid grid-cols-[1fr_80px] gap-2"><input type="number" min="0" step="any" className={inputClass} value={form.sale} onChange={(e) => set("sale", e.target.value)} /><input className={inputClass} maxLength={3} value={form.salesCurrency} onChange={(e) => set("salesCurrency", e.target.value.toUpperCase())} /></div></Field></div><Field label="Notes"><textarea className={inputClass} value={form.notes} onChange={event => set("notes", event.target.value)} /></Field>{error ? <p className="text-sm text-red-300">{error}</p> : null}<button disabled={pending || Boolean(data.deal.archived_at)} className="inline-flex items-center gap-2 rounded-md bg-foreground px-3.5 py-2 text-xs font-medium text-background disabled:opacity-40"><Plus className="h-3.5 w-3.5" />{editingId ? "Save product line" : "Add product line"}</button>{editingId ? <button type="button" className="ml-3 text-xs underline" onClick={() => { setEditingId(null); setForm(emptyLine); setError(null); }}>Cancel edit</button> : null}</form></div>;
}
